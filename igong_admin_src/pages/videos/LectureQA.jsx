...
import { mockConsultations, mockVideos } from '../../utils/mockData'

export default function LectureQA() {
  const [qaList, setQaList] = useState(
    mockConsultations.map(c => ({ ...c, type: '강의Q&A', lecture: mockVideos[c.id % mockVideos.length].title }))
  )
  ...
  const openReply = (qa) => { setSelectedQA(qa); setReplyText(''); setReplyModal(true) }
  const saveReply = () => {
    setQaList(prev => prev.map(q => q.id === selectedQA.id ? { ...q, answered: true, reply: replyText, replyDate: new Date().toISOString().slice(0, 10) } : q))
    setReplyModal(false)
  }
  const deleteQA = (id) => { if (confirm('삭제하시겠습니까?')) setQaList(prev => prev.filter(q => q.id !== id)) }
  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.tableWrap}>
        <table style={s.table}>
          <thead>
            <tr>{['번호', '이름', '아이디', '닉네임', '강의명', '내용', '등록일', '답변상태', '관리'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {paged.map(q => (
              <tr key={q.id} style={styles.tr}>
                ...
                <td style={{ ...styles.td, maxWidth: 160 }}><div style={styles.truncate}>{q.lecture}</div></td>
                <td style={{ ...styles.td, maxWidth: 200 }}><div style={styles.truncate}>{q.content}</div></td>
                ...
                <td style={styles.td}>
                  ...
                  <button style={{ ...styles.btnSm, ...styles.btnDanger }} onClick={() => deleteQA(q.id)}>삭제</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {replyModal && selectedQA && (
        <div style={s.overlay}>
          <div style={s.modal}>
            <div style={s.modalTitle}>Q&A 답변 작성</div>
            <div style={s.qBox}>
              <div style={s.qLabel}>질문</div>
              <div style={s.qContent}>{selectedQA.content}</div>
              <div style={s.qMeta}>{selectedQA.name} ({selectedQA.userId}) · {selectedQA.regDate}</div>
            </div>
            <div style={s.mf}>
              <label style={s.ml}>답변 내용</label>
              <textarea style={s.textarea} rows={5} value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="답변을 입력하세요..." />
            </div>
            <div style={s.mbtn}>
              <button style={s.btnPrimary} onClick={saveReply}>저장</button>
...
const s = {
  ...
  qLabel: { fontSize: 11, fontWeight: 700, color: '#7c3aed', marginBottom: 8, textTransform: 'uppercase' },
  qContent: { fontSize: 14, color: '#1a1a2e', lineHeight: 1.6 },
  qMeta: { fontSize: 11, color: '#888', marginTop: 8 },
